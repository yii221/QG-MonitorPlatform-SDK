package com.pmpsdk.utils;

import org.junit.jupiter.api.Test;

import javax.crypto.SecretKey;
import javax.crypto.spec.SecretKeySpec;
import java.security.KeyPair;
import java.security.PrivateKey;
import java.security.PublicKey;
import java.util.Base64;

/**
 * @Description: 加密测试类  // 类说明
 * @ClassName: CryptoUtil    // 类名
 * @Author: lrt          // 创建者
 * @Date: 2025/8/5 19:39   // 时间
 * @Version: 1.0     // 版本
 */
public class CryptoUtilTest {

    private static final String PUBLIC_KEY = "";
    private static final String PRIVATE_KEY = "";


    @Test
    public void testCryptoUtil() throws Exception {
        SecretKey aesKey = CryptoUtil.generateAESKey(256);
        String s = CryptoUtil.aesEncrypt("Hello, World", aesKey);
        System.out.println("Encrypted: " + s);
        String decrypted = CryptoUtil.aesDecrypt(s, aesKey);
        System.out.println("Decrypted: " + decrypted);
    }

    @Test
    public void testRsaEncryptDecrypt() throws Exception {
        String data = "Hello, RSA Encryption!";

        SecretKey key = CryptoUtil.generateAESKey(256);// 生成AES密钥

        // 使用AES密钥加密数据
        String encryptedData = CryptoUtil.aesEncrypt(data, key);
        System.out.println("Encrypted Data: " + encryptedData);


        // 生成RSA密钥对
        PublicKey publicKey = CryptoUtil.getPublicKeyFromString(PUBLIC_KEY);
        PrivateKey privateKey = CryptoUtil.getPrivateKeyFromString(PRIVATE_KEY);

        // 使用RSA公钥加密AES密钥
        String encryptedKey = CryptoUtil.rsaEncryptAESKey(key, publicKey);
        System.out.println("Encrypted AES Key: " + encryptedKey);
        // 使用RSA私钥解密AES密钥
        SecretKey decryptedKey = CryptoUtil.rsaDecryptAESKey(encryptedKey, privateKey);
        System.out.println("Decrypted AES Key: " + Base64.getEncoder().encodeToString(decryptedKey.getEncoded()));
        // 使用解密后的AES密钥解密数据
        String decryptedData = CryptoUtil.aesDecrypt(encryptedData, decryptedKey);
        System.out.println("Decrypted Data: " + decryptedData);

    }

}
